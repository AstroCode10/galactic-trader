...
bool Game::load_game() {
    ...
    for (int i = 0; i < n_ships; i++) {
        ...
        player.set_ship_owned(name, is_owned);
    }
...
void Game::market_menu(int choice) {
    ...
    do {
        ...
        switch (market_action) {
            case 1: {
                ...
                if (resource_choice != 4) {
                    do {
                        ...
                        if (number <= 0) {
                            std::cout << "Please enter a positive number." << std::endl << std::endl;
                            continue;
                        }
            ...
            default:
                std::cout << "Please enter a valid action (1 - 4)" << std::endl;
                break;
        }
...
void Game::check_random_encounter(std::mt19937 &eng) {
    ...
    if (encounter_roll <= 40) {
        ...
        handle_solar_flare(eng);
    } else if (encounter_roll <= 90) {
        std::cout << "[EVENT]: DERELICT CARGO SALVAGE POOL!" << std::endl;
        handle_salvage(eng);
    } else if (encounter_roll <= 98) {
        std::cout << "[EVENT]: SPACE PIRATE AMBUSH!" << std::endl;
        handle_pirate_event(eng);
    } else {
        std::cout << "[EVENT]: WANDERING ALIEN MERCHANT!" << std::endl;
        handle_alien_merchant();
    }
}

void Game::handle_debris(std::mt19937 &eng) {
    std::cout << "\nCLANG!" << std::endl;
    std::cout << "Minor structural damage has occurred to your hull" << std::endl;

    int upper_repair = std::max(10, static_cast<int>(1.5 * player.get_credits()));
    std::uniform_int_distribution<int> repair_dist(10, upper_repair);
    ...
    if (player.get_credits() < repair_cost) {
        ...
        std::uniform_int_distribution<int> item_lost_dist(
            player.get_credits() > 0 ? 5 : 20,
            player.get_credits() > 0 ? 20 : 60
        );
        ...
        double inv_lost_modifier = item_lost_dist(eng) / 100.0;
        int total_cargo = player.get_total_item_num();
        int slots_lost = static_cast<int>(inv_lost_modifier * total_cargo);
        if (slots_lost < 1 && total_cargo > 0) {
            slots_lost = 1;
        }

        std::uniform_int_distribution<int> iron_lost_dist(0, slots_lost);
...
void Game::handle_solar_flare(std::mt19937 &eng) {
    std::cout << "\nThe control panels flicker wildly" << std::endl;
    std::cout << "An emergency ventilation sequence was activated" << std::endl;
...
void Game::handle_salvage(std::mt19937 &eng) {
    ...
    do {
        ...
        std::cout << "1. Investigate (Costs " << DERELICT_FUEL << " units of fuel to match velocities and dock)" << std::endl;
    ...
    std::uniform_int_distribution<int> success_dist(1, 5);
    int success = success_dist(eng);
    ...
    if (success <= 4) {
        ...
        std::uniform_int_distribution<int> percent_dist(20, 50);
        double salvage_modifier = percent_dist(eng) / 100.0;
        ...
        std::uniform_int_distribution<int> iron_found_dist(0, units_found);
        int iron_found = iron_found_dist(eng);
        int water_found = units_found - iron_found;

        std::cout << "SUCCESS! You cracked open the vault. Salvage breakdown:" << std::endl;
        if (iron_found > 0) std::cout << "   + " << iron_found << " units of pure Iron Ore" << std::endl;
        if (water_found > 0) std::cout << "   + " << water_found << " units of Vaporized Water Cargo" << std::endl;
        std::cout << "Your inventory has been automatically updated." << std::endl;
        ...
        int upper_repair = std::max(60, static_cast<int>(1.1 * player.get_credits()));
        std::uniform_int_distribution<int> repair_dist(60, upper_repair);
        int repair_cost = repair_dist(eng);
        ...
        if (player.get_credits() < repair_cost) {
            ...
            std::uniform_int_distribution<int> item_lost_dist(
                player.get_credits() > 0 ? 5 : 30,
                player.get_credits() > 0 ? 30 : 70
            );
            ...
            std::uniform_int_distribution<int> idx_dist(0, 1);
            int item_idx = idx_dist(eng);
            double item_lost_modifier = item_lost_dist(eng) / 100.0;
            int num_lost = static_cast<int>(item_lost_modifier * player.get_item(item_idx));
            if (num_lost < 1 && player.get_item(item_idx) > 0) {
                num_lost = 1;
            }

            if (item_idx == 0) std::cout << "You lost " << num_lost << " units of Iron blocks" << std::endl;
            if (item_idx == 1) std::cout << "You lost " << num_lost << " units of Water" << std::endl;

            player.remove_item(item_idx, num_lost);

        } else {
            player.remove_credits(repair_cost);
            std::cout << "You paid " << repair_cost << " credits for immediate hull repairs." << std::endl;
        }
    }
}
...
void Game::handle_pirate_event(std::mt19937 &eng) {
    ...
    std::string active_ship_name = ship.get_type();
    ...
    int demand_amount = percentage_toll > FLAT_TOLL ? percentage_toll : FLAT_TOLL;
    ...
    do {
        ...
        std::cout << "1. Pay the toll (" << demand_amount << " Star Coins)" << std::endl;
    ...
    if (choice == 1) {
        if (player.get_credits() >= demand_amount) {
            player.remove_credits(demand_amount);
            std::cout << "You transfer " << demand_amount << " Star Coins. The pirates vanish into the void." << std::endl;
        } else {
            std::cout << "You don't have enough Star Coins! The pirates seize half your cargo instead." << std::endl;
        ...
        int escape_roll = escape_dist(eng);
...
void Game::handle_alien_merchant() {
    int choice;
    int gallons;
    int cost;
    ...
    do {
        ...
        std::cout << "2. I also tapped an antimatter hydrogen pocket and will fill your tanks right now" << std::endl;
    ...
    if (choice == 1) {
        int iron_amount = player.get_item(0);
        int water_amount = player.get_item(1);

        std::cout << "Transferring " << iron_amount << " units of Iron Ore and "
                  << water_amount << " units of Water to Xylar..." << std::endl;

        int amount = 2 * (iron_amount * 120 + water_amount * 80);
        player.set_item(0, 0);
        player.set_item(1, 0);
        ...
        do {
            ...
            if (player.get_fuel() + gallons > ship.get_max_fuel()) {
                std::cout << "Oops! You only have " << ship.get_max_fuel() - player.get_fuel()
                          << " empty gallons of fuel left in your tank." << std::endl;
                std::cout << "Try again." << std::endl;
            } else if (player.get_credits() < cost) {
                std::cout << "Oops! You only have " << player.get_credits() << " Star Coins." << std::endl;
                std::cout << "You need " << cost - player.get_credits()
                          << " more Star Coins to complete the transaction." << std::endl;
        ...
        std::cout << "Refilled " << gallons << " gallons of fuel" << std::endl;
