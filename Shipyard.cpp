...
void Shipyard::show_shipyard_menu(Player &player, Ship &curr_ship) {
    ...
    do {
        ...
        if (choice == 1) {
            ...
            if (player.get_credits() >= curr_ship.get_damage_costs()){
                ...
                std::cout << "You need " << curr_ship.get_damage_costs() - player.get_credits()
...
void Shipyard::repair_ship(Player &player, Ship &curr_ship){
    std::cout << "Success! Completely fixed hull for " << curr_ship.get_damage_cost() << std::endl;
    player.remove_credits(ship.get_damage_cost())
    curr_ship.set_damage_cost(0);
    curr_ship.set_max_cargo(catalog[ship.get_ship_type()][1]);
}